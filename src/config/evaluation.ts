...
const LOCATION_ELIGIBILITY_FILTER: EvaluationFilter = {
  name: "location-eligibility",
  prompt: `You are a strict location and work-authorization filter. Your ONLY job is to determine whether the candidate can plausibly pursue this job. Ignore tech stack, seniority, compensation, and domain quality unless the rules below explicitly use "exceptional AI lab/frontier AI" as a tie-breaker.
...
The word "Remote" anywhere (in ATS metadata or in the body) describes WORK MODE, not GEOGRAPHIC SCOPE. To override a country restriction you need an explicit geographic-scope signal in the body per the rules below — a body that just says "fully remote" or "Remote" repeatedly is NOT a global signal.
...
PASS: "We are a fully remote team distributed across Europe." → true EU remote ✓
...
PASS: "Remote EU, annual company meetup in Berlin" → EU remote + occasional business meeting ✓
FAIL: "Remote - United States" → US remote blocked
FAIL: "Remote, must be authorized to work in the United States" → US authorization blocked
...
[Country / authorization handling]
FAIL: ATS Workplace type=Remote, country=United States, locations=[Remote], body discusses role/stack but is silent on geo → US remote/authorization risk
FAIL: ATS Workplace type=Remote, country=US, locations=[Remote], body says "Remote" multiple times and lists US-style benefits (medical/dental, 401k) but no geographic-scope statement → Remote is work mode, not global eligibility
PASS: ATS Workplace type=Remote, locations=[Canada, Remote], country=Canada, body says "be part of a high-performing, globally distributed team" → global team overrides single-country ATS
PASS: ATS Workplace type=Remote, locations=[United States], country=United States, body says "we hire globally regardless of location, employees in 25+ countries" → body says global
PASS: ATS Workplace type=Remote, locations="Spain" only → true EU remote
PASS: ATS Workplace type=Remote, locations="Spain, Portugal, Germany" → true EU remote
